...
import { fetchGlobalActivity, fetchTeam } from '../lib/api'
...
export function DashboardPage() {
  ...
  // Alert strip computations from in-memory data.
  ...
  // Stuck cases: in-progress checklists with no toggled item activity in 7+ days.
  // We approximate per spec: in-progress (some items checked, not all) checklists
  // whose most-recent activity entry is >7 days old; if no activity exists at all
  // and the checklist itself is >7 days old, also count it.
  const stuckCaseIds = useMemo(() => {
    const checklistRecency = new Map<string, string>()
    for (const entry of activity) {
      if (typeof entry.target === 'string') {
        // best-effort: target may contain checklist id; record most recent timestamp
  ...
  // Unbilled hours: total billable minutes this month from data.timeEntries.
  // Approximation: prototype has no "invoice sent" state, so this counts all
  ...
  // My queue: owner's own assigned checklists not complete, grouped by due bucket.
  ...
  // Team workload: non-owner employees.
  const teammates = data.employees.filter((e) => e.role !== 'Owner')

  // Active cases in flight: distinct caseIds, with at least one stage completed
  // and at least one stage not completed.
  ...
  // KPI strip: hours tracked, billable %, projected billing, active clients.
