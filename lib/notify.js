 ...
 *
 * `desktopUrl` is set ONLY when the sign-in request came from the desktop
 * shell (the request-link handler spots the shell's user-agent marker). A
 * magic link clicked in a mail client opens the default BROWSER — which
 * signs the browser in, not the shell — so the shell's email carries a
 * second button on the pbjsa:// scheme, which Windows routes to the
 * installed app. Browser sign-ins never see that button.
 *
 * params: { firmName, signInUrl, desktopUrl? } → { subject, html }
 */
export function buildLoginLinkEmail({ firmName, signInUrl, desktopUrl }) {
  const safeFirm = String(firmName || 'PB&J Strategic Accounting')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
  const subject = `Sign in to ${safeFirm}`
  const desktopBlock = desktopUrl
    ? `
    <p style="margin:0 0 20px;"><a href="${desktopUrl}" style="display:inline-block;background:#1f1d1a;color:#fff;font-weight:600;text-decoration:none;padding:12px 22px;border-radius:8px;font-size:15px;">Open in the desktop app</a></p>
    <p style="margin:0 0 20px;font-size:12px;color:#7d7269;">You asked to sign in from the desktop app — use the dark button to sign in there. The pink button signs in your browser instead.</p>`
    : ''
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;color:#1f1d1a;background:#f6f5f1;padding:24px;">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:28px;box-shadow:0 8px 32px rgba(31,29,26,0.06);">
    ...
    <p style="margin:0 0 20px;font-size:15px;line-height:1.5;color:#555049;">Click the button below to sign in. This link expires in 15 minutes and can only be used once.</p>${desktopBlock}
 ...
 *
 * params: { to, firmName, signInUrl, desktopUrl? }
 */
export async function sendLoginLinkEmail({ to, firmName, signInUrl, desktopUrl }) {
  if (!to || !signInUrl) return
  if (!process.env.RESEND_API_KEY || !process.env.EMAIL_FROM) return

  const { subject, html } = buildLoginLinkEmail({ firmName, signInUrl, desktopUrl })
 ...
 */
export async function sendInvoiceEmail({ to, subject, html, text, attachments = [] }) {
  const apiKey = process.env.RESEND_API_KEY
  const from = process.env.INVOICE_EMAIL_FROM || process.env.EMAIL_FROM
  if (!apiKey || !from) {
    return { ok: false, error: 'Email is not configured yet (no sending address set).' }
  }
