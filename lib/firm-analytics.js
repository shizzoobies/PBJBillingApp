 ...
 * no hourly wage. This matches the Client Recap and the payroll report, which
 * is the point — the assistant must not tell her a margin is unavailable for a
 * client whose Recap page is showing her a number. `marginAvailable` is kept
 * as a constant true so an older caller can't break on its absence.
 */
...
export function clientProfitability(data, { month, costRates = {}, lowRealizationThreshold = 50 }) {
  ...
  for (const client of clients) {
    ...
    // and the payroll report both count it as zero; this is the same rule, so
    // the assistant can't contradict the page it is describing.
    let laborCost = 0
    for (const e of mine) {
      const rate = costRates[e.employeeId]
      if (rate != null) laborCost += hoursOf(e) * rate
    }
