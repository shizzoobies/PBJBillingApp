 ...
 *     which only a client's own longer terms can stretch
 *
...
export function dueDateFromTerms(issueDate, terms, windowDays = DEFAULT_PAYMENT_WINDOW_DAYS) {
  const parsed = parseNetDays(terms)
...
/**
 * The terms line to PRINT, so a document agrees with its own due date.
 *
 * A client's stored wording is shown only when it actually describes the window
 * they got — when it parses to a `Net N` at or beyond the firm's. Everything
 * else ("Due on receipt", "Due on Demand", blank, prose) prints the window the
 * invoice was really dated by, because printing "Due on receipt" next to a date
 * thirty days out is the contradiction this replaces.
 */
export function paymentTermsLabel(terms, windowDays = DEFAULT_PAYMENT_WINDOW_DAYS) {
  const parsed = parseNetDays(terms)
  if (parsed !== null && parsed >= windowDays) return String(terms).trim()
