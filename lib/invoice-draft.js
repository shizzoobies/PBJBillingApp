...
export function dueDateFromTerms(issueDate, terms, windowDays = DEFAULT_PAYMENT_WINDOW_DAYS) {
  ...
  // UTC throughout. `${issueDate}T12:00:00` is parsed in the HOST's zone and
  // then read back with `toISOString()`, so the same invoice generated in
  // Chicago and stamped on a Railway box in UTC could land a day apart — and a
  // past-due line is compared as a string against a UTC day. Midday keeps a DST
  // shift from moving the date; `Z` keeps the host out of it entirely.
