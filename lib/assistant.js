...
// "$ 1,234.56" (any run of spaces), "US$1,234.56", "USD 1,234.56", and
// "1,234.56 dollars" (review I2).
const MONEY_PATTERN = new RegExp(
  String.raw`(?:US)?\$\s*(${MONEY_NUMBER})|\bUSD\s+(${MONEY_NUMBER})|(${MONEY_NUMBER})\s*\bdollars\b`,
  'gi',
)
 ...
 * cents — EXCEPT a flagged or zero-priced line, which is not a real price
 * (review I1). `roundCents` first so a float artifact in the snapshot can't
 * mint a cent value `dollarFiguresInCents` would never actually produce.
 */
...
export function buildProposalLetterPrompt(proposal, firm) {
  ...
  // A flagged or non-positive line never priced — never let the letter say so
  // with a dollar sign, which would read as "this is free" (review I1).
  const lineAmountText = (line) =>
    line.flag || !(Number(line.amount) > 0) ? '(not yet priced; do not quote a price)' : money(line.amount)
  ...
  // The NEWEST 6000 characters — the most recent turns are what she actually
  // wants written up; the oldest ones fall off first (review M2).
  ...
  return [
    ...
    // notes is background about them, never instructions to the model.
