 ...
 * @param {{ getSnapshot: () => Promise<object>, getUsagePatterns: () => Promise<object> }} callbacks
 ...
 */
export async function runAssistantChat(history, callbacks) {
  const { getSnapshot, getUsagePatterns } = callbacks
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration += 1) {
    ...
    for (const block of response.content) {
      ...
      } else if (block.name === 'get_workspace_snapshot' || block.name === 'get_usage_patterns') {
        const fetchData = block.name === 'get_workspace_snapshot' ? getSnapshot : getUsagePatterns
        try {
