...
const OWNER_FEEDBACK_SCHEMA = {
  type: 'object',
  properties: { confirmation: { type: 'string' }, forDeveloper: { type: 'string' } },
 ...
 * schema twice — before throwing the friendly 502.
 *
 * @param {object} client
 ...
 * @param {{ endpoint: string, unavailable: string, invalid: string }} labels
 */
async function runStructuredModel(client, params, validate, labels) {
  let lastText = ''
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    let response
    try {
      response = await runModel(client, params)
    } catch (error) {
    ...
    if (lastStopReason === 'refusal') {
      throw Object.assign(new Error('The AI declined that request — try rephrasing.'), {
        statusCode: 502,
      })
    }
