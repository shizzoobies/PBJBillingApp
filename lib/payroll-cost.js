...
export function laborCost(entries, costRateOf, duplicates = duplicateFullSliceIds(entries)) {
  // Each person's ROW MINUTES, not a running total: the costing hours are the
  // sum of the rows' two-decimal hours, so collapsing them here would throw
  // away the very thing being summed. See {@link periodDisplayHours}.
  const rowsByEmployee = new Map()
  for (const entry of entries) {
    if (duplicates.has(entry.id)) continue
    const found = rowsByEmployee.get(entry.employeeId)
    if (found) found.push(entry.minutes)
    else rowsByEmployee.set(entry.employeeId, [entry.minutes])
  }
  return sumPersonCosts(
    [...rowsByEmployee].map(([employeeId, minutesPerRow]) =>
      periodMoney(minutesPerRow, costRateOf(employeeId)),
    ),
  )
}
