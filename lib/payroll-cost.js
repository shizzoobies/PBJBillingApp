 ...
 * from it. The negative figures in the app are the Client Recap's OVER/UNDER
 * cells — every role row's hours variance and the Total under them, plus the
 * profit variance beside them — where an underrun is negative. The asymmetry
 * is a half-hundredth of an hour and invisible at the two decimals printed;
 * noted so nobody has to rediscover it. Those cells are built by SUBTRACTING
 * two already-rounded figures (`sumDisplayHours`), never by rounding a raw
 * negative, so in practice nothing reaches this function negative today.
 */
 ...
 *
 * `null` = the person has NO cost rate — a real state, not a half-filled form:
 * their time carries no labor cost. Never render it as $0.00. Role has no say
 * in this; an owner who sets a cost rate on the Team page (featreq-6fdd9e98)
 * is priced right here with everyone else.
 */
/**
 * NOT FOR ACTUALS — see {@link periodMoney}.
 *
 * This prices a single already-total hours figure, which is right for a
 * FORECAST (the Client Recap's estimated hours per role: one number somebody
 * typed, with no rows behind it). Handed a period's worth of real minutes it
 * rounds the total instead of the rows, which is precisely the defect
 * featreq-7c8f64d7 came back for. Actuals have rows; use them.
 */
...
/**
 * THE COSTING HOURS for one person over one period: the sum of the two-decimal
 * hours of each ROW, which is exactly the figure printed at the bottom of the
 * Hours column.
 *
 * This is the correction at the heart of featreq-7c8f64d7's fourth round.
 * `displayHours(totalMinutes)` and this are NOT the same number, and the report
 * was printing one while charging the other:
 *
 *   Allison, 8/8-8/22, 31 entries — rows sum to 14.75h, the raw total rounds to
 *   14.78h. Cost was 14.78 x 38 = $561.64 under a column that read 14.75.
 *   Lisa, same period, 63 entries — rows sum to 22.61h, raw rounds to 22.59h,
 *   so hers was too LOW by the same mechanism.
 *
 * That opposite-direction pair is why three previous passes read as a rounding
 * bug and were not: nothing here rounds the wrong way, the two figures are
 * simply computed from different inputs. The owner adds the column and
 * multiplies what she sees, so what she sees is the multiplicand.
 */
export function periodDisplayHours(minutesPerRow) {
  return sumDisplayHours((Array.isArray(minutesPerRow) ? minutesPerRow : []).map(displayHours))
}

/**
 * Money for one person over one period: {@link periodDisplayHours} x their rate,
 * settled to the cent.
 *
 * Used for BOTH sides of the report — the cost rate for what the firm pays, the
 * bill rate for what the client owes — because the owner checks them the same
 * way: take the hours on the row, multiply by the rate, compare. One function
 * means the two can never drift apart.
 *
 * `null` = no rate on file, which is a real answer rather than a missing
 * setting. Never render it as $0.00.
 */
export function periodMoney(minutesPerRow, rate) {
  if (typeof rate !== 'number' || Number.isNaN(rate)) return null
  return roundToCent(periodDisplayHours(minutesPerRow) * rate)
}
...
export function allocatePersonCost(minutesPerRow, rate) {
  ...
  const targetCents = Math.round(periodMoney(minutesPerRow, rate) * 100)
...
export function laborCost(entries, costRateOf, duplicates = duplicateFullSliceIds(entries)) {
  return groupedMoney(
    entries,
    (entry) => !duplicates.has(entry.id),
    (entry) => costRateOf(entry.employeeId, entry.date),
  )
}

/**
 * THE OTHER SIDE OF THE SAME RULE: what a set of slices BILLS for — each
 * person's billable hours × their bill rate as it stands for the client the
 * hour was worked for.
 *
 * `billRateOf` is `(entry) => number | null` and receives the whole row, because
 * the bill side needs more than the person and the day: the rate is the
 * person's version at the ENTRY'S CLIENT'S pin for the entry's month
 * (`billRateAt(versions, employee, ratePeriodAsOf(client, month), month)`).
 * Two clients on two pins bill the same person's September hour at two
 * different figures, and this is where that lands on the Reports page.
 *
 * Grouped by person AND resolved rate exactly as {@link laborCost} is, and for
 * the same reason: the rows at each rate are their own two-decimal sum, so the
 * per-person figure ties to the per-entry cells and to the invoice lines the
 * generator would write. Deliberately NOT deduped: a full-mode group split bills
 * each client the whole block, and this is the billing side of the report —
 * the same reason the Billable hours column beside it is not deduped either.
 *
 * Only BILLABLE rows count; a person with no billable rows, or none with a
 * rate, contributes nothing — the caller decides whether that is "$0.00" or
 * "—" (see `periodCostOf` on the Reports page for the pattern).
 */
export function billableRevenue(entries, billRateOf) {
  return groupedMoney(entries, (entry) => entry.billable === true, billRateOf)
}

/**
 * Both totals above, once: the rows that `include` says count, grouped by
 * person and the rate `rateOf` resolves for each, each group's ROW MINUTES
 * kept (not a running total — the hours are the sum of the rows' two-decimal
 * hours, so collapsing them here would throw away the very thing being
 * summed), and the groups' cent-rounded money added under {@link sumPersonCosts}.
 */
function groupedMoney(entries, include, rateOf) {
