...
describe('the generated draft', () => {
  it('carries the ad hoc lines, their mode and a matching reserve amount', () => {
    ...
    // REPINNED for featreq-cfb1536a (her answer, revised in person to A): the
    // line prints "0.83h at $100/hr", so it charges 0.83 × 100 = $83.00 — the
    // printed hours ARE the price. It used to charge the raw 50 minutes
    // ($83.33), a number the detail text could never reproduce by hand. Both
    // fields still land on the same cent, so a courtesy-then-billed round trip
    // cannot move the total by a penny.
    expect(adhoc[0].amount).toBe(83)
    expect(adhoc[0].adhocAmount).toBe(83)
    expect(adhoc[0].adhocMode).toBe('billed')
    expect(draft.total).toBe(283)
  })
})
