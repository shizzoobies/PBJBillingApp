...
/*
 * THE PAYMENT WINDOW (Alex, 2026-09-15). Every invoice is due 30 days after
 * the day it is ISSUED. The rule this replaced counted from the PERIOD END and
 * honored "due on receipt" literally, so the sixteen clients whose records say
 * that were emailed an invoice on Sept 1 that had been due since Aug 31.
 */
describe('dueDateFromTerms', () => {
  it('gives terms that name no window the firm window from the issue date', () => {
    expect(dueDateFromTerms('2026-09-01', 'Due on receipt')).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', 'Immediate')).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', 'Due on Demand')).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', '')).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', undefined)).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', 'whenever you get to it')).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', 'Net 9999')).toBe('2026-10-01')
  })

  // The window is a FLOOR. A client who wrote a shorter one does not get a
  // shorter one — the firm gives everybody the same thirty days.
  it('lifts a shorter written term up to the window', () => {
    expect(dueDateFromTerms('2026-09-01', 'Net 15')).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', 'net10')).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', '15 days')).toBe('2026-10-01')
    expect(dueDateFromTerms('2026-09-01', 'Net 30')).toBe('2026-10-01')
  })

  it('honors a longer contractual window', () => {
    expect(dueDateFromTerms('2026-09-01', 'NET 45')).toBe('2026-10-16')
    expect(dueDateFromTerms('2026-09-01', '60 days')).toBe('2026-10-31')
  })

  it('defaults to the firm constant and still takes a caller window', () => {
    expect(DEFAULT_PAYMENT_WINDOW_DAYS).toBe(30)
    expect(dueDateFromTerms('2026-09-01', '')).toBe(
      dueDateFromTerms('2026-09-01', '', DEFAULT_PAYMENT_WINDOW_DAYS),
    )
    expect(dueDateFromTerms('2026-09-01', 'Net 15', 45)).toBe('2026-10-16')
  })

  it('returns null for an unreadable issue date', () => {
    expect(dueDateFromTerms('nonsense', 'Net 30')).toBeNull()
  })
})

/*
 * WHAT THE CLIENT IS TOLD. The due date above is the FIRM’s: the day after
 * which an invoice is chased. The client is asked to pay on receipt, because a
 * date on the document reads as permission to wait a month. Only a client whose
 * record names a LONGER window sees a date, and it is their own.
 */
describe('customerNetDays and paymentTermsLabel', () => {
  it('asks for payment on receipt when the record names no window', () => {
    for (const terms of ['Due on receipt', 'Immediate', 'Due on Demand', '', undefined, 'whenever you get to it', 'Net 9999']) {
      expect(customerNetDays(terms)).toBeNull()
      expect(paymentTermsLabel(terms)).toBe('Due on receipt')
    }
  })

  // A shorter written term buys the client nothing and costs them nothing: the
  // stored due date was floored to the firm’s thirty days, so printing “Net 15”
  // beside it would contradict it. They are asked to pay on receipt instead.
  it('asks for payment on receipt when the record names a SHORTER window', () => {
    expect(customerNetDays('Net 15')).toBeNull()
    expect(paymentTermsLabel('Net 15')).toBe('Due on receipt')
    expect(paymentTermsLabel('15 days')).toBe('Due on receipt')
  })

  it('keeps a longer window in the client’s own words', () => {
    expect(customerNetDays('Net 45')).toBe(45)
    expect(paymentTermsLabel('Net 45')).toBe('Net 45')
    expect(paymentTermsLabel('  60 days  ')).toBe('60 days')
  })

  // The label and the date are one decision: whatever window is honored is the
  // window the document prints.
  it('moves with a caller-supplied window, exactly as the date does', () => {
    expect(paymentTermsLabel('Net 15', 45)).toBe('Due on receipt')
    expect(paymentTermsLabel('Net 60', 45)).toBe('Net 60')
    expect(customerNetDays('Net 30', 45)).toBeNull()
  })

  it('reads the raw Net N a record names, window or not', () => {
    expect(parsedNetDays('Net 15')).toBe(15)
    expect(parsedNetDays('net30')).toBe(30)
    expect(parsedNetDays('Due on receipt')).toBeNull()
    expect(parsedNetDays('Net 9999')).toBeNull()
    expect(parsedNetDays('')).toBeNull()
  })
})
...
describe('buildInvoiceDraft', () => {
  it('produces lines, totals and a due date for a flat-fee client', () => {
    ...
    // Thirty days from the day it is issued, NOT from the end of the month it
    // bills for — August's invoice goes out on Sept 1 and is due Oct 1.
  ...
  it('bills an hourly client per employee and rounds to cents', () => {
    ...
    // "Net 15" on the record, thirty days on the invoice: the window is a floor.
    expect(draft.dueDate).toBe('2026-10-01')
  })
})
...
describe('buildConsolidatedInvoiceDraft', () => {
  ...
  it('takes the due date from the MASTER’s payment terms, not a sub’s', () => {
    ...
    // The master's "Net 15" is lifted to the firm's thirty days, counted from
    // the issue date — the subs' own due dates are irrelevant either way.
    expect(draft.dueDate).toBe('2026-10-01')
  })
