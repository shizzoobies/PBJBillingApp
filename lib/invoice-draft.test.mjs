...
/*
 * The printed line has to agree with the date beside it — a sheet reading "Due
 * on receipt" under a due date thirty days out is the contradiction the window
 * exists to remove.
 */
describe('paymentTermsLabel', () => {
  it('prints the window for terms that do not describe one', () => {
    expect(paymentTermsLabel('Due on receipt')).toBe('Net 30')
    expect(paymentTermsLabel('Due on Demand')).toBe('Net 30')
    expect(paymentTermsLabel('')).toBe('Net 30')
    expect(paymentTermsLabel(undefined)).toBe('Net 30')
    expect(paymentTermsLabel('whenever you get to it')).toBe('Net 30')
  })

  it('prints the window a shorter written term was actually billed on', () => {
    expect(paymentTermsLabel('Net 15')).toBe('Net 30')
    expect(paymentTermsLabel('15 days')).toBe('Net 30')
  })

  it('keeps a longer term in the client’s own words', () => {
  ...
  it('moves with a caller-supplied window, exactly as the date does', () => {
    expect(paymentTermsLabel('Net 15', 45)).toBe('Net 45')
