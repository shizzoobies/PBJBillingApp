...
import { renderedInvoiceLines } from './invoice-lines.js'
...
const ITEM_BY_KIND = {
  ...
  custom: 'Services',
  // One-off work outside the client's scope. Same placeholder as the rest —
  // it is service revenue, just tracked separately on our side.
  adhoc: 'Services',
  ...
  'card-fee': 'Services',
  // The two ends of a retainer. They share one item name on purpose: in QBO
  // these belong together and net to nothing over the life of the engagement —
  // money taken in up front, then given back against the work it was held for.
  // The credit exports as a NEGATIVE ItemAmount, which is what makes the final
  // invoice import for what is actually owed.
  retainer: 'Retainer',
  retainer_credit: 'Retainer',
}
...
export function buildQboCsv(invoices, clientsById) {
  ...
  for (const invoice of invoices ?? []) {
    ...
    // Omitted ad hoc lines never reach QBO: they are not on the client's
    // invoice, so a receivable row for one would be money nobody was asked for.
    // A courtesy line does export, at $0.00 — it IS on their invoice.
    for (const line of renderedInvoiceLines(invoice.lineItems)) {
