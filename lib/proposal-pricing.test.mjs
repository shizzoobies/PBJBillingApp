...
describe('applyProposalPatch', () => {
  ...
  it('appends new notes to the existing notes instead of replacing them (I1)', () => {
    const withNotes = { ...current, prospect: { ...current.prospect, notes: 'Uses QBO.' } }
    const next = applyProposalPatch(withNotes, { prospect: { notes: 'Wants monthly reports.' } }, services)
    expect(next.prospect.notes).toBe('Uses QBO.\nWants monthly reports.')
  })

  it('does not append a note that is already contained in the existing notes (I1)', () => {
    const withNotes = {
      ...current,
      prospect: { ...current.prospect, notes: 'Uses QBO. Wants monthly reports.' },
    }
    const next = applyProposalPatch(withNotes, { prospect: { notes: 'Wants monthly reports.' } }, services)
    expect(next.prospect.notes).toBe('Uses QBO. Wants monthly reports.')
  })

  it('stores the addition as-is when it already repeats the existing notes, instead of doubling them (task 13c #1)', () => {
    const withNotes = { ...current, prospect: { ...current.prospect, notes: 'A' } }
    const next = applyProposalPatch(withNotes, { prospect: { notes: 'A\nB' } }, services)
    expect(next.prospect.notes).toBe('A\nB')
  })

  it('caps appended notes at 4000 chars (task 13c #1)', () => {
    const withNotes = { ...current, prospect: { ...current.prospect, notes: 'A'.repeat(3990) } }
    const next = applyProposalPatch(withNotes, { prospect: { notes: 'B'.repeat(50) } }, services)
    expect(next.prospect.notes).toHaveLength(4000)
    expect(next.prospect.notes.endsWith('B'.repeat(9))).toBe(true)
  })

  it('two tierless rows sharing a group and name coexist (M5)', () => {
    ...
    const start = { prospect: {}, inputs: {}, selections: [{ serviceId: 'row-a' }] }
    const next = applyProposalPatch(
      start,
      { selections: { add: [{ serviceId: 'row-b' }], remove: [] } },
      customServices,
    )
    expect(next.selections.map((row) => row.serviceId).sort()).toEqual(['row-a', 'row-b'])
  })

  it('re-adding a selected row keeps its override (M7)', () => {
    const withOverride = { ...current, selections: [{ serviceId: 'reconciliations', override: 250 }] }
    const next = applyProposalPatch(
      withOverride,
      { selections: { add: [{ serviceId: 'reconciliations', quantity: 3 }], remove: [] } },
      services,
    )
    const row = next.selections.find((selection) => selection.serviceId === 'reconciliations')
    expect(row.override).toBe(250)
    expect(row.quantity).toBe(3)
  })

  it('a row no longer in the catalog can still be removed (M7)', () => {
    const withGhost = { ...current, selections: [...current.selections, { serviceId: 'ghost-service' }] }
    const next = applyProposalPatch(
      withGhost,
      { selections: { add: [], remove: ['ghost-service'] } },
      services,
    )
    expect(next.selections.map((row) => row.serviceId)).not.toContain('ghost-service')
  })
})
