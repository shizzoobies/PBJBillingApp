...
// at the client's pin exactly as a generated one would.
import { billRateAt } from './rate-history.js'
...
function scopeLineTools({
  ...
  // `buildInvoiceLines.rateFor`, resolved at the client's pin for this month
  // (`ratePeriod`, else the invoice's own period) — never the person's live
  // `billRate` while they have versions, because that mirrors the NEWEST one.
 ...
 * @param {string|null} [args.ratePeriod] - the client's pin for `period`
 *   (`ratePeriodAsOf(client, period)`); null prices at `period` itself.
