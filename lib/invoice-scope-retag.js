...
// THE bill-rate chain the generator prices with, so a line this creates bills
// at the client's pin exactly as a generated one would — and THE pin rule, so
// the pin is the ENTRY's client's, which on a billing master is a sub's.
...
function scopeLineTools({
  ...
  // The clients an entry can belong to: the invoice's own, plus the roster the
  // caller hands over — which is what lets a MASTER's panel find each sub.
  const clientById = new Map()
  if (client && typeof client.id === 'string') clientById.set(client.id, client)
  for (const known of Array.isArray(clients) ? clients : []) {
    if (known && typeof known.id === 'string') clientById.set(known.id, known)
  }
  /**
   * THE PIN IS THE ENTRY'S CLIENT'S, NOT THE INVOICE'S. A billing master holds
   * no pin of its own; each sub keeps one, and the generator prices each sub's
   * hours at that sub's pin before the merge. A line this creates on the
   * master's invoice has to price the same way, or re-tagging one entry on the
   * KLC invoice would bill it at the invoice month (today's rates) while the
   * generator billed the hours beside it at the sub's pin. So the period is
   * resolved per entry, from the entry's own client. `ratePeriod` — the
   * invoice's own pin, which callers already pass — is the fallback for an
   * entry whose client is not on the roster.
   */
  const ratePeriodFor = (entry) => {
    const own = entry ? clientById.get(entry.clientId) : undefined
    if (own) return ratePeriodAsOf(own, period || null)
    return ratePeriod ?? ratePeriodAsOf(client, period || null)
  }
  // `buildInvoiceLines.rateFor`, resolved at the ENTRY's client's pin for this
  // month (else the invoice's own period) — never the person's live `billRate`
  // while they have versions, because that mirrors the NEWEST one.
  const rateFor = (entry) =>
    billRateAt(
      billRateVersions,
      employeeById.get(entry.employeeId) ?? { id: entry.employeeId },
      ratePeriodFor(entry),
      period || null,
    ) ?? defaultHourlyRate
  ...
  const findAdhoc = (list, entry) => {
    ...
    const detail = adhocDetail(entry, employee, rateFor(entry))
    return list.findIndex(
      (line) =>
...
export function savedAdhocModesForEntries({
  ...
  const { findAdhoc } = scopeLineTools({
    entries: rows,
    employees,
    client,
    clients,
 ...
 * @param {Array<object>} [args.clients] - the roster, so an entry's rate period
 *   is its OWN client's pin — on a billing master, the sub's — resolved through
 *   `ratePeriodAsOf(sub, period)` exactly as the generator prices each sub.
 * @param {string} args.period - YYYY-MM.
 * @param {number} args.defaultHourlyRate - fallback when an employee has none.
 * @param {Array<object>} [args.billRateVersions] - dated bill rates; a NEW line
 *   is priced through `billRateAt` exactly as the generator prices one.
 * @param {string|null} [args.ratePeriod] - the invoice's own pin for `period`
 *   (`ratePeriodAsOf(client, period)`), used only for an entry whose client is
 *   not in `clients`; null prices at `period` itself.
...
export function applyScopeRetag({
  ...
  for (const entryId of ids.slice().sort()) {
    ...
    if (to === 'in-scope') {
      ...
      if (line) {
        const hoursNow = Number(line.hours)
        const rate = Number.isFinite(Number(line.rate)) ? Number(line.rate) : rateFor(entry)
    ...
    if (to === 'adhoc') {
      ...
      if (index >= 0) {
        ...
        const rate = departedRate ?? rateFor(entry)
