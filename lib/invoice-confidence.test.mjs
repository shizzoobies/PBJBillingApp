...
describe('rateInvoiceDraft', () => {
  ...
  // REWRITTEN 2026-09-22 for the claude-opus-5-5 bump: its structured-output
  // validator rejects minimum/maximum on integers and minItems/maxItems on
  // arrays with a 400, which would 502 every rating. The caps moved into the
  // validator; this pins that the schema never grows one of those keywords
  // back, and that the validator still enforces the caps the schema used to.
  it('sends a schema with no bounds keywords — the 5.5 validator rejects them', async () => {
    const offenders = []
    const walk = (node, path) => {
      if (!node || typeof node !== 'object') return
      for (const [key, value] of Object.entries(node)) {
        if (['minimum', 'maximum', 'minItems', 'maxItems'].includes(key)) {
          offenders.push(`${path}.${key}`)
        }
        walk(value, `${path}.${key}`)
      }
    }
    walk(INVOICE_CONFIDENCE_SCHEMA, 'schema')
    expect(offenders).toEqual([])
  })

  it('caps concerns at 4 and questions at 3 in the validator, not the schema', async () => {
    const oversized = {
      ...goodVerdict,
      concerns: Array.from({ length: 6 }, (_, i) => ({
        line: `Line ${i + 1}`,
        issue: `Something worth a second look on line ${i + 1}`,
        severity: 'info',
      })),
      questions: Array.from({ length: 5 }, (_, i) => `Question number ${i + 1} for the owner?`),
    }
    const anthropic = fakeClient([jsonResponse(oversized)])
    const result = await rateInvoiceDraft(input, { client: anthropic })
    expect(result.concerns).toHaveLength(4)
    expect(result.questions).toHaveLength(3)
    expect(anthropic.messages.create).toHaveBeenCalledTimes(1)
  ...
  it('honors INVOICE_AI_MODEL and otherwise uses claude-opus-5-5', async () => {
    expect(resolveInvoiceModel()).toBe('claude-opus-5-5')
