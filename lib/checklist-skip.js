...
//
// IT IS NOT GATED BY `skipAllowed`. Push shipped sharing skipping's per-template
// opt-in, and that made it invisible: 6 of 150 active templates carry the flag.
// Moving a date is not stepping past work, so it needs no per-recipe permission
// — see {@link canOfferPush}. Skipping is unchanged.
//
// What makes a push safe on a RECURRING task is NOT in this file: the instance
// keeps its place in the cycle by stamping `cycleDueDate` with its original
// date, which is what the materializer's identity reads
// (lib/checklist-identity.js). Without that, pushing to the next cycle's date
// would both respawn the cycle just left and collide with the occurrence being
// pushed into. A one-off has no cycle and no materializer identity, so the same
// stamp is simply inert on it.
...
/**
 * Should the PUSH affordance be rendered? On EVERY task the viewer can edit.
 *
 * It used to ride skipping's `skipAllowed` opt-in, which reads sensible and was
 * wrong in practice: 6 of 150 active templates have that flag, so the button
 * Brittany asked for was invisible on 95% of tasks and she reported it as "I
 * cannot find the push button" (featreq-68638ed2). The two are not the same
 * permission. SKIPPING steps past an occurrence — work that was supposed to
 * happen does not, which is why an owner opts into it per recipe. PUSHING moves
 * a date; the task stays open, nothing is written off, and everyone who hears
 * about a skip hears about the push. Nobody needs permission to say "this is
 * happening on the 20th instead".
 *
 * One-off tasks included, for the same reason: a one-off has no cycle to keep a
 * place in, which is what made it unSKIPPABLE, but it has a due date like
 * anything else and the dialog already falls back to +1 month when there is no
 * schedule to read a next occurrence from.
 *
 * An already-skipped instance is still closed out for its cycle, so it cannot
 * be pushed.
 */
export function canOfferPush({ checklist, canWrite }) {
  if (!canWrite) return false
  return !isChecklistSkipped(checklist)
}
