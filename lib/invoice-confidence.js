 ...
 *
 * Model: `INVOICE_AI_MODEL` or `claude-opus-5-5` (bumped from claude-opus-5
 * on 2026-09-22 alongside the assistant), with `modelFallback: false`.
...
export const DEFAULT_INVOICE_AI_MODEL = 'claude-opus-5-5'
...
export const INVOICE_CONFIDENCE_SCHEMA = {
  ...
  properties: {
    ...
    // No minimum/maximum here: claude-opus-5-5's structured-output validator
    // rejects those keywords on integers (400 "properties maximum, minimum are
    // not supported"); the 0-100 range is enforced by the validator below,
    // which clamps and re-bands. The prompt still states the range.
    score: { type: 'integer' },
    summary: { type: 'string', minLength: 20 },
    // No maxItems on the arrays either (same 5.5 validator rule as `score`
    // above): the validator below truncates to MAX_CONCERNS / MAX_QUESTIONS.
    concerns: {
      type: 'array',
      items: {
        type: 'object',
