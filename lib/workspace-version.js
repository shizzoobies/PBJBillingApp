 ...
 *
 * `invoice_drafts` is deliberately ABSENT. A bulk save cannot destroy it: the
 * table is not part of the payload, and `write()` snapshots its rows before the
 * wipe (needed only because its `client_id` FK is `on delete restrict`) and
 * restores them afterwards. Nothing a stale tab holds can change a draft, so
