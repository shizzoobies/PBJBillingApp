 ...
 *
 * `invoices` is deliberately ABSENT (as was the `invoice_drafts` placeholder it
 * replaces). A bulk save cannot destroy it: the table is not part of the
 * payload, and `write()` snapshots its rows before the wipe (needed only
 * because its `client_id` FK is `on delete restrict`) and restores them
 * afterwards. Nothing a stale tab holds can change an invoice, so
 * fingerprinting it would invalidate every open tab for writes the guard has no
 * business gating — including, once invoicing ships, every tab open while
 * Brittany works through a month's drafts.
 *
...
/**
 * Columns a bulk save can NEVER change, dropped from the digest per table.
 *
 * The guard exists to stop a stale tab overwriting things. A column the tab
 * cannot write is therefore a column whose churn can only produce false 409s —
 * exactly the reasoning that keeps `invoices` out of `BULK_SAVE_TABLES`
 * entirely, applied at column granularity because `recurring_reimbursements`
 * genuinely IS in the payload for its other fields.
 *
 * The three covered-date columns here are written by invoice GENERATION and by
 * the confirm control, and `store.write()` snapshots and restores them rather
 * than taking the payload's copy (see `preservedCoverage`). Without this, a
 * month run would invalidate every tab Brittany had open — and it is precisely
 * when working through a month's invoices that she has them open.
 *
 * PRESERVING IS WHAT MAKES EXCLUDING SAFE. These two halves must move together:
 * excluding a column the bulk save could still overwrite would reopen the wipe
 * this guard was built for.
 */
export const VERSION_IGNORED_COLUMNS = {
  recurring_reimbursements: ['coverage_history', 'coverage_anchor_day', 'coverage_resume_pending'],
}

/** The same set, in the file backend's camelCase, keyed by slice. */
export const VERSION_IGNORED_FIELDS = {
  recurringReimbursements: ['coverageHistory', 'coverageAnchorDay', 'coverageResumePending'],
}
 ...
 * - anything in `VERSION_IGNORED_COLUMNS` is dropped for the reason given there.
...
export function tableVersionSql(table) {
  const dropped = ['updated_at', 'created_at', ...(VERSION_IGNORED_COLUMNS[table] ?? [])]
    .map((column) => ` - '${column}'`)
    .join('')
  return `select md5(coalesce(string_agg(x, ',' order by x), '')) as h
            from (select (to_jsonb(t)${dropped})::text as x
                    from ${table} t) s`
}
...
/**
 * Drop the churn-only timestamps and the slice's server-owned fields, matching
 * the Postgres branch's `- 'column'` chain.
 */
function withoutTimestamps(record, slice) {
  if (!record || typeof record !== 'object' || Array.isArray(record)) return record
  const { updatedAt: _updatedAt, createdAt: _createdAt, ...rest } = record
  for (const field of VERSION_IGNORED_FIELDS[slice] ?? []) delete rest[field]
  return rest
}
...
export function fileWorkspaceVersion(data) {
  ...
  for (const slice of BULK_SAVE_SLICES) {
    ...
    const rendered = rows.map((row) => stableStringify(withoutTimestamps(row, slice))).sort()
