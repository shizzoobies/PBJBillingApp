...
/** Cap on `prospect.notes` after a chat patch appends to it - matches
 *  `cleanProposalProspect`'s own cap so the two never disagree. */
const PROSPECT_NOTES_CHARS = 4000
 ...
 *
 * Every prospect field REPLACES except `notes`, which APPENDS (a chat patch
 * must never erase what she already wrote in the notes) - existing text plus
 * a newline plus the new text, trimmed and capped, and never appended twice
 * when the new text is already there.
 *
 * Adding one tier of a tiered row replaces its siblings (same group + name),
 * but ONLY when both the added row's tier and the sibling's tier are
 * non-null - two tierless rows sharing a group + name are different services
 * and both stay selected.
 */
export function applyProposalPatch(current, patch, services) {
  ...
  for (const [field, value] of Object.entries(patch?.prospect ?? {})) {
    if (typeof value !== 'string' || !Object.hasOwn(prospect, field)) continue
    if (field === 'notes') {
      const addition = value.trim()
      const existing = prospect.notes
      if (addition && !existing.includes(addition)) {
        // The model sometimes hands back the existing notes plus the new
        // sentence rather than just the new sentence (spec says it should
        // send only what she said THIS turn) - when the addition already
        // contains the existing text, store the addition as-is instead of
        // appending it onto the existing text a second time.
        const next = addition.includes(existing.trim())
          ? addition
          : existing
            ? `${existing}\n${addition}`
            : addition
        prospect.notes = next.slice(0, PROSPECT_NOTES_CHARS)
      }
    } else {
  ...
  for (const add of patch?.selections?.add ?? []) {
    ...
    const siblingIds = new Set([
      ...
      ...(service.tier !== null
        ? catalog
            .filter((row) => row.group === service.group && row.name === service.name && row.tier !== null)
            .map((row) => row.id)
        : []),
    ])
