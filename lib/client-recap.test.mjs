...
import { buildClientRecap, buildMasterRecap } from './client-recap.js'
...
describe('buildClientRecap — staff (no financials)', () => {
  const r = buildClientRecap(data, { ...ownerOpts, includeFinancials: false, costRateVersions: [] })
...
describe('buildClientRecap — labor cost off the printed hours', () => {
  ...
  const r = buildClientRecap(costData, {
    ...
    includeFinancials: true,
    costRateVersions: [
      { userId: 'e1', effectiveDate: '1970-01-01', rate: 22.5 },
      { userId: 'e2', effectiveDate: '1970-01-01', rate: 31.75 },
    ],
  })
...
describe('buildClientRecap — fixed staff order', () => {
  ...
  const order = (snapshot, period) =>
    buildClientRecap(snapshot, { ...ownerOpts, period, costRateVersions: [] }).time.byStaff.map(
      (row) => row.name,
    )
  ...
  it('omits a tier nobody logged time in, and the rest keep their order', () => {
    ...
    const rows = buildClientRecap(snapshot, { ...ownerOpts, costRateVersions: [] }).time.byStaff
  ...
  it('reports hours at two decimals, not one', () => {
    ...
    const r = buildClientRecap(snapshot, { ...ownerOpts, costRateVersions: [] })
...
describe('buildClientRecap — the role table adds up', () => {
  ...
  describe('staff see the actual column and none of the plan', () => {
    const staff = buildClientRecap(roleData, { ...opts, includeFinancials: false, costRateVersions: [] })
    ...
    it('drops a role that was estimated but never worked — nothing is left to say', () => {
      const cfo = buildClientRecap(
        ...
        { ...opts, includeFinancials: false, costRateVersions: [] },
      )
...
/**
 * The KLC billing master's recap: "recap at each sub and then master level".
 *
 * The contract under test is that this ADDS and never recomputes — a master
 * holds no time of its own, so every figure has to be traceable to a sub recap
 * that already exists. Each assertion is therefore written against the SUB
 * recaps rather than against a hand-computed constant: if the per-client math
 * ever changes, these must move with it rather than pinning an old answer.
 */
describe('buildMasterRecap', () => {
  const master = { id: 'm1', name: 'KLC Master' }
  const sub = (clientId, over = {}) =>
    buildClientRecap(data, { ...ownerOpts, clientId, ...over })

  const clover = sub('c1')
  const northstar = sub('c2')
  const rolled = buildMasterRecap({ master, subRecaps: [clover, northstar] })

  it('is the master, and names the subs behind it', () => {
    expect(rolled.client).toEqual({ id: 'm1', name: 'KLC Master', billingMode: null })
    expect(rolled.isBillingMaster).toBe(true)
    expect(rolled.subs).toEqual([
      { id: 'c1', name: 'Clover' },
      { id: 'c2', name: 'Northstar' },
    ])
    expect(rolled.period).toBe('2026-08')
    expect(rolled.periodLabel).toBe(clover.periodLabel)
  })

  it('sums billed revenue and reimbursements', () => {
    expect(rolled.billing.revenue).toBe(clover.billing.revenue + northstar.billing.revenue)
    expect(rolled.billing.reimbursementTotal).toBe(
      clover.billing.reimbursementTotal + northstar.billing.reimbursementTotal,
    )
    // A master has no billing machinery of its own.
    expect(rolled.billing.billingMode).toBeNull()
    expect(rolled.billing.hourlyRate).toBeNull()
    // Each reimbursement says which company it belongs to.
    expect(rolled.billing.reimbursements[0].client).toBe('Clover')
  })

  it('sums labor cost and margin', () => {
    expect(rolled.profitability.laborCost).toBe(
      clover.profitability.laborCost + northstar.profitability.laborCost,
    )
    expect(rolled.profitability.margin).toBe(
      clover.profitability.margin + northstar.profitability.margin,
    )
  })

  it('sums hours, and merges a person who works more than one company', () => {
    expect(rolled.time.totalHours).toBe(clover.time.totalHours + northstar.time.totalHours)
    expect(rolled.time.billableHours).toBe(
      clover.time.billableHours + northstar.time.billableHours,
    )
    // Avery logged 11h on Clover and 2h on Northstar — one person, 13 hours.
    const avery = rolled.time.byStaff.filter((row) => row.name === 'Avery')
    expect(avery).toHaveLength(1)
    expect(avery[0].hours).toBe(13)
    expect(rolled.time.roleTotals.actualHours).toBe(rolled.time.totalHours)
  })

  it('sums the estimate side, and says which subs had nothing planned', () => {
    const estimated = buildClientRecap(
      {
        ...data,
        clients: data.clients.map((c) =>
          c.id === 'c1' ? { ...c, estimatedBookkeeperHours: 8 } : c,
        ),
      },
      ownerOpts,
    )
    const withPlan = buildMasterRecap({ master, subRecaps: [estimated, northstar] })
    expect(withPlan.estimates.hasEstimate).toBe(true)
    expect(withPlan.estimates.hours.estimated).toBe(estimated.estimates.hours.estimated)
    expect(withPlan.estimates.unestimatedSubs).toEqual(['Northstar'])
    expect(withPlan.estimates.profit.actualRevenue).toBe(withPlan.billing.revenue)
    // A rate cannot be rolled up; the costs beside it still are.
    expect(withPlan.estimates.byTier.every((row) => row.costRate === null)).toBe(true)
  })

  it('sums the task counts and tags each task with its company', () => {
    expect(rolled.tasks.dueCount).toBe(clover.tasks.dueCount + northstar.tasks.dueCount)
    expect(rolled.tasks.overdueCount).toBe(
      clover.tasks.overdueCount + northstar.tasks.overdueCount,
    )
    expect(rolled.tasks.dueThisPeriod[0].client).toBe('Clover')
  })

  // A staff payload that reported three companies' money because the fourth
  // was redacted would be a worse leak than the redaction prevents.
  it('withholds every financial section unless EVERY sub carried one', () => {
    const staffSub = sub('c2', { includeFinancials: false, costRateVersions: [] })
    const mixed = buildMasterRecap({ master, subRecaps: [clover, staffSub] })
    expect(mixed.includeFinancials).toBe(false)
    expect(mixed.billing).toBeNull()
    expect(mixed.profitability).toBeNull()
    expect(mixed.estimates).toBeNull()
    // The operational side still rolls up.
    expect(mixed.time.totalHours).toBe(clover.time.totalHours + staffSub.time.totalHours)
  })

  it('rolls up nothing that cannot honestly be added', () => {
    expect(rolled.salesTax).toBeNull()
    expect(rolled.projection).toBeNull()
  })

  it('returns null for a master with no subs — that is misconfiguration, not a zero', () => {
    expect(buildMasterRecap({ master, subRecaps: [] })).toBeNull()
    expect(buildMasterRecap({ master: null, subRecaps: [clover] })).toBeNull()
  })
})
