...
export function buildCheckoutLineItems(invoice, client) {
  ...
  return (
    lines
      ...
      // happens to keep an ad hoc line the owner omitted (and a courtesy line,
      // which is $0.00 by decision) off the payment page. That is the right
      // outcome, but it is a side effect: if this guard is ever relaxed, filter
      // through `renderedInvoiceLines` instead so an omitted line cannot appear
