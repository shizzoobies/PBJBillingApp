 ...
 *
 * THE LINES COME THROUGH `clientFacingInvoiceLines`, NOT OFF THE INVOICE. Stripe
 * renders these on its own hosted page and repeats them in its own receipt
 * email, so this is a client-facing surface exactly like the PDF and the email —
 * it just isn't one we draw. A billing master sending raw lines here would show
 * KLC every sub company's name and coverage window on the payment page, which is
 * the one thing Brittany's answer rules out, and no amount of care in our own
 * two renderers would have caught it.
 */
export function buildCheckoutLineItems(invoice, client) {
  ...
  return (
    lines
      // Zero-amount lines are dropped because Stripe rejects them, which also
      // keeps a courtesy line ($0.00 by decision) off the payment page. An
      // OMITTED ad hoc line is no longer relying on this: `clientFacingInvoiceLines`
      // above filters through `renderedInvoiceLines`, so it is gone before this
      // guard sees it, and relaxing this can no longer put one in front of a
      // client.
 ...
 */
async function createSession({ invoice, client, customerId, appUrl, channel, returnTo = null }) {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      // `returnTo` is the invoice's own PUBLIC pay page (`/pay/<token>`). The
      // default below points at the AUTHENTICATED app, so a client who has just
      // handed over their bank details lands on a sign-in screen they can never
      // get past. Absent, these are byte-for-byte what they always were.
      success_url: returnTo
        ? `${returnTo}?paid=1`
        : `${appUrl}/invoices?paid=${encodeURIComponent(invoice.id)}`,
      cancel_url: returnTo
        ? `${returnTo}?cancelled=1`
        : `${appUrl}/invoices?cancelled=${encodeURIComponent(invoice.id)}`,
    })
...
export async function createInvoiceCheckoutSession({
  ...
  return createSession({ invoice, client, customerId, appUrl, channel: 'ach', returnTo })
}
 ...
 * @returns {{ok: true, session: object} | {ok: false, reason: string}}
 */
export async function createInvoiceCardCheckoutSession({
  invoice,
  client,
  customerId,
  appUrl,
  ...
  return createSession({ invoice, client, customerId, appUrl, channel: 'card', returnTo })
}
 ...
 */
/**
 * What Stripe says about a payment intent RIGHT NOW — for reconciling an
 * invoice stuck in 'processing' when the settle webhook was lost or applied
 * out of order (INV-2026-08-003 sat that way for 12 days: succeeded arrived
 * before completed, and completed wrote 'processing' over 'paid').
 *
 * Read-only. Returns { status, chargedAt } or null when Stripe is not
 * configured / the intent cannot be fetched — the caller says "could not
 * reach Stripe", it does not guess.
 */
export async function retrievePaymentIntentStatus(paymentIntentId) {
  ...
  try {
    ...
    console.warn(
      `[stripe] could not retrieve intent ${paymentIntentId}:`,
      error?.message || error,
    )
    return null
  }
