...
describe('a billing master', () => {
  ...
  /**
   * A master holds no pin of its own, so `ratePeriodAsOf(master, period)` is
   * null and the old code priced a re-tagged line at the INVOICE month — the
   * raise — while the generator had priced the hours beside it at each sub's
   * own pin. Two subs on two pins: each new line has to carry its own sub's
   * rate, on both arrivals, and the un-stamped ad hoc match has to look for
   * the detail at that rate too.
   */
  describe('prices each sub at its OWN pin, never at the invoice month', () => {
    // Patrice: $100 from June, raised to $120 from August.
    const billRateVersions = [
      { userId: 'emp-1', effectivePeriod: '2026-06', rate: 100 },
      { userId: 'emp-1', effectivePeriod: '2026-08', rate: 120 },
    ]
    // Sub A is still on June's rates; sub B was moved to August's.
    const SUBS = [
      { id: 'client-a', name: 'A', billingMode: 'hourly', hourlyRatePeriod: '2026-06' },
      { id: 'client-b', name: 'B', billingMode: 'hourly', hourlyRatePeriod: '2026-08' },
    ]
    const outOfScope = [
      entry({ id: 'a1', clientId: 'client-a', billable: false, description: 'Catch-up A' }),
      entry({ id: 'b1', clientId: 'client-b', billable: false, description: 'Catch-up B' }),
    ]
    const pinnedRetag = (tagEdits, over = {}) =>
      applyScopeRetag({
        lines: [],
        entries: outOfScope,
        tagEdits,
        ...
        defaultHourlyRate: 60,
        billRateVersions,
        // What the editor passes for the invoice itself — null on a master.
        ratePeriod: null,
        ...over,
      })

    it('to in scope: one line per company, each at that company’s rate', () => {
      const { lines } = pinnedRetag({ a1: { tag: 'in-scope' }, b1: { tag: 'in-scope' } })
      const hourly = lines.filter((line) => line.kind === 'hourly')
      expect(hourly).toHaveLength(2)
      expect(hourly.find((line) => line.sourceClientId === 'client-a')).toMatchObject({
        rate: 100,
        amount: 100,
        detail: '1.00h at $100.00/hr',
      })
      expect(hourly.find((line) => line.sourceClientId === 'client-b')).toMatchObject({
        rate: 120,
        amount: 120,
        detail: '1.00h at $120.00/hr',
      })
    })

    it('to ad hoc: the same two rates, on the ad hoc lines', () => {
      const { lines } = pinnedRetag({
        a1: { tag: 'adhoc', adhocMode: 'billed' },
        b1: { tag: 'adhoc', adhocMode: 'billed' },
      })
      expect(find(lines, 'Adhoc — Catch-up A')).toMatchObject({
        amount: 100,
        adhocAmount: 100,
        sourceClientId: 'client-a',
      })
      expect(find(lines, 'Adhoc — Catch-up B')).toMatchObject({
        amount: 120,
        adhocAmount: 120,
        sourceClientId: 'client-b',
      })
    ...
    it('a sub’s pin as it stood THAT month, read off its ledger', () => {
      // B was moved to August's rates in July; a July invoice still bills it
      // at June's, exactly as the generator would have.
      const moved = {
        ...SUBS[1],
        hourlyRateHistory: [
          { from: '2026-06', to: '2026-08', changedAt: '2026-07-15T12:00:00.000Z', changedBy: 'emp-1' },
        ],
      }
      const { lines } = pinnedRetag(
        { b1: { tag: 'in-scope' } },
        { clients: [MASTER, SUBS[0], moved], period: '2026-07' },
      )
      expect(lines.find((line) => line.sourceClientId === 'client-b')).toMatchObject({
        rate: 100,
      })
    })

    it('finds an un-stamped ad hoc line by the detail at the sub’s rate', () => {
      // A pre-commit draft's ad hoc line carries no entryId; the match is by
      // label + detail, and the detail prints the rate the line was made at —
      // the sub's pin, not the invoice month's.
      const adhocB = entry({
        id: 'b2',
        clientId: 'client-b',
        isAdhoc: true,
      ...
      const lines = [
        {
          kind: 'adhoc',
          label: 'Adhoc — Rush B',
          detail: 'Aug 6, 2026 · Patrice · 1.00h at $120.00/hr',
          amount: 120,
          adhocMode: 'courtesy',
          adhocAmount: 120,
          sourceClientId: 'client-b',
