...
/** A 1970 floor is what the rate-history migration writes for today's rates. */
const flatCostRates = (rates) =>
  Object.entries(rates).map(([userId, rate]) => ({
    userId,
...
describe('clientProfitability', () => {
  it('computes realization and margin, worst realization first, excluding archived', () => {
    const result = clientProfitability(data, {
      month: '2026-06',
      costRateVersions: flatCostRates({ e1: 30, e2: 50 }),
  ...
  it('prices a contributor with no cost rate at zero rather than withholding margin', () => {
    // e2 has no cost rate. Anyone can be in that state — the owner was, for as
    // long as the Team page refused her the box — and she logs on nearly every
    // client, so withholding margin here blanked almost every row and
    // contradicted the Recap page, which counts it as zero.
    const result = clientProfitability(data, {
      month: '2026-06',
      costRateVersions: flatCostRates({ e1: 30 }),
    })
    const c1 = result.clients.find((c) => c.client === 'Fixed Co')
    expect(c1.marginAvailable).toBe(true)
  ...
  /**
   * featreq-6fdd9e98 — an OWNER with a cost rate costs money like anyone else.
   *
   * "I need to input a cost for me so I can budget." The Team page now offers
   * every member the Cost rate box, so the owner row is no longer permanently
   * blank. Nothing in here had to change for that, which is the point worth
   * pinning: this function has always asked the RATE ON FILE, never the role,
   * and the second half of the test keeps the empty case honest.
   */
  it('costs an owner who has a cost rate, and still charges nothing for one who has not', () => {
    const withOwnerTime = {
      ...data,
      timeEntries: [
        ...data.timeEntries,
        { employeeId: 'owner', clientId: 'c1', date: '2026-06-10', minutes: 120, billable: true },
      ],
    }

    const priced = clientProfitability(withOwnerTime, {
      month: '2026-06',
      costRateVersions: flatCostRates({ e1: 30, e2: 50, owner: 90 }),
    })
    const c1 = priced.clients.find((c) => c.client === 'Fixed Co')
    expect(c1.totalHours).toBe(17)
    expect(c1.laborCost).toBe(730) // 10*30 + 5*50 + her own 2h at 90
    expect(c1.margin).toBe(270) // 1000 fee - 730

    const blank = clientProfitability(withOwnerTime, {
      month: '2026-06',
      // "No cost rate on file" is now the ABSENCE of a version row, which is
      // what the rate table actually holds for somebody who never had one.
      costRateVersions: flatCostRates({ e1: 30, e2: 50 }),
    })
    const unpriced = blank.clients.find((c) => c.client === 'Fixed Co')
    expect(unpriced.laborCost).toBe(550) // her 2h still cost nothing
    expect(unpriced.margin).toBe(450)
  })

  it('returns nothing for a month with no activity', () => {
    const result = clientProfitability(data, { month: '2025-01', costRateVersions: [] })
...
describe('clientProfitability labor cost parity with the payroll rule', () => {
  ...
  const costRateVersions = flatCostRates({ p1: 8.01, p2: 8.01 })
  ...
  it('rounds per person to the cent, the way payroll pays — not sum-then-round2', () => {
    const result = clientProfitability(parityData, { month: '2026-08', costRateVersions })
  ...
  /**
   * featreq-7c8f64d7: the rule became "round the hours to two decimals, THEN
   * multiply". The assistant reads these rows out loud, so if it lagged the
   * payroll report the owner would be told two different margins for the same
   * month. It inherits the change because it calls `laborCost` — this pins that
   * it actually did.
   */
  it('inherits round-the-hours-first from the shared rule', () => {
    const secondsPrecise = {
      clients: [{ id: 'a', name: 'Alpha', billingMode: 'hourly', hourlyRate: 100 }],
      employees: [{ id: 'p1', name: 'Pat', role: 'Bookkeeper' }],
      // 613.4 min = 10.2233h, printed 10.22h.
      timeEntries: [
        { id: 't1', employeeId: 'p1', clientId: 'a', date: '2026-08-05', minutes: 613.4, billable: true },
      ],
      checklists: [],
    }
    const result = clientProfitability(secondsPrecise, {
      month: '2026-08',
      costRateVersions: flatCostRates({ p1: 22.5 }),
    })
    // 10.22 × 22.5 = $229.95. Dividing seconds gave $230.03.
    expect(result.clients[0].laborCost).toBe(229.95)
    expect(result.clients[0].laborCost).not.toBe(230.03)
    // And the hours it quotes are the hours that cost was priced from. These
    // were one decimal, so the assistant said "10.2 hours" beside a figure
    // built from 10.22 — two surfaces disagreeing about the same hour.
    expect(result.clients[0].totalHours).toBe(10.22)
    expect(result.clients[0].totalHours).not.toBe(10.2)
  })
})

/**
 * The assistant reads these lists out loud. A total that contradicts the rows
 * it just read is the same defect as one on a printed report.
 */
describe('timeSummary totals the rows it returns', () => {
  const secondsPrecise = {
    clients: [
      { id: 'a', name: 'Alpha', billingMode: 'hourly', hourlyRate: 100 },
      { id: 'b', name: 'Beta', billingMode: 'hourly', hourlyRate: 100 },
    ...
    timeEntries: [
      { id: 't1', employeeId: 'p1', clientId: 'a', date: '2026-08-05', minutes: 10, billable: true },
      { id: 't2', employeeId: 'p2', clientId: 'b', date: '2026-08-06', minutes: 10, billable: true },
      { id: 't3', employeeId: 'p1', clientId: 'a', date: '2026-08-07', minutes: 45, billable: true },
    ],
    checklists: [],
  }

  it('reports two-decimal hours, matching every page', () => {
    const r = timeSummary(secondsPrecise, { from: '2026-08-01', to: '2026-08-31', groupBy: 'staff' })
    expect(r.byStaff.map((s) => s.hours).sort()).toEqual([0.17, 0.92])
  })

  it('adds the rows it just listed, not the seconds behind them', () => {
    const r = timeSummary(secondsPrecise, { from: '2026-08-01', to: '2026-08-31', groupBy: 'both' })
    // Pat 55 min → 0.92h, Quinn 10 min → 0.17h; the list adds to 1.09h where
    // the raw 65 minutes round to 1.08h.
    expect(r.totalHours).toBe(1.09)
    expect(r.totalHours).toBe(r.byStaff.reduce((sum, s) => sum + Math.round(s.hours * 100), 0) / 100)
    expect(r.totalHours).toBe(r.byClient.reduce((sum, c) => sum + Math.round(c.hours * 100), 0) / 100)
  })
})
