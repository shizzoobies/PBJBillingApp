...
const ownerOpts = {
  ...
  costRateVersions: [{ userId: 'lisa', effectiveDate: '1970-01-01', rate: 40 }],
}
...
describe('estimated vs actual — profit uses the shared calculators', () => {
  ...
  it('averages the tier rate when the assigned people cost different amounts', () => {
    ...
    const t = buildClientRecap(twoBookkeepers, {
      ...ownerOpts,
      costRateVersions: [
        { userId: 'lisa', effectiveDate: '1970-01-01', rate: 40 },
        { userId: 'dana', effectiveDate: '1970-01-01', rate: 60 },
      ],
    })
   ...
   * of the 34 clients with any time, and had no cost rate on file. The old rule
   * withheld cost and profit whenever ANY contributor lacked a rate, which
   * would have blanked this panel on 91% of her clients. A rate-less person
   * contributes zero instead, exactly as personPeriodCost and the payroll
   * report have always treated them.
   */
  it('prices a rate-less role at zero instead of withholding the estimate', () => {
    const noRate = buildClientRecap(lisaData, { ...ownerOpts, costRateVersions: [] })
  ...
  /**
   * One client, one person on it, and that person is the OWNER. Shared by the
   * two tests below so the only thing that differs between them is her rate.
   */
  const ownerOnly = {
    ...lisaData,
    clients: [
  ...
  /**
   * The rate-less owner, kept as a case rather than as a law.
   *
   * She had no cost rate because the Team page never offered her the box; since
   * featreq-6fdd9e98 it does, so this now pins the EMPTY half of the rule — a
   * member with no rate, whatever their role, costs nothing — and the test
   * below pins the other half with the same fixture and a rate filled in.
   */
  it('still shows profit for a client whose ONLY time is a rate-less owner', () => {
    // She has NO ROW in the rate history, which is how "no rate on file" looks
    // now that rates are effective-dated — the exact production shape.
    const r = buildClientRecap(ownerOnly, { ...ownerOpts, costRateVersions: [] })
  ...
  /**
   * featreq-6fdd9e98 — the same client once the OWNER has entered her own cost.
   *
   * Brittany's ask was "I need to input a cost for me so I can budget", so her
   * hours have to land in cost and profit, not just in the hours columns. Her
   * tier prices from her rate on BOTH sides: the estimate at 8 CFO hours and
   * the actual at the 10 she worked.
   */
  it('costs an owner who HAS a cost rate, on both sides of the comparison', () => {
    const r = buildClientRecap(ownerOnly, {
      ...ownerOpts,
      costRateVersions: [{ userId: 'brittany', effectiveDate: '1970-01-01', rate: 60 }],
    })
    const cfo = r.estimates.byTier.find((row) => row.tier === 'CFO')
    expect(cfo.costRate).toBe(60)
    expect(cfo.costRateBasis).toBe('assigned')
    expect(cfo.actualCost).toBe(600) // 10.00h × $60
    expect(r.profitability.laborCost).toBe(600)
    expect(r.profitability.margin).toBe(900) // 1500 revenue - 600
    expect(r.estimates.profit.estimatedCost).toBe(480) // 8h × $60
    expect(r.estimates.profit.estimatedProfit).toBe(720) // 1200 - 480
    expect(r.estimates.profit.actualCost).toBe(600)
    expect(r.estimates.profit.actualProfit).toBe(900)
  })
})
