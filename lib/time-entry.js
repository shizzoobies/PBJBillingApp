 ...
 *
 * ONE further exemption: an OWNER changing nothing but the SCOPE FLAGS —
 * `isAdhoc` and `billable`. Approval asks "is this record of the work right",
 * and neither flag touches that: together they decide how the time BILLS (in
 * scope, out of scope, ad hoc). The person flipping them is the approver, and
 * the review surface is where they are meant to flip them, so revoking their own
 * sign-off would make the backstop yank the row out of the list they are
 * working through. Anyone else's scope edit still re-queues, and so does an
 * owner's the moment it carries any other field.
 *
 * `billable` joined `isAdhoc` here with the hours panel beside the invoice
 * (featreq-8cec48db), which offers the same three-way choice from the invoicing
 * side and writes it without touching approval at all. Leaving the Time page's
 * rule narrower would have meant the same decision cost an entry its sign-off on
...
export function editRequiresReapproval(approvalStatus, patch = {}, isOwner = false) {
  ...
  return Object.keys(patch ?? {}).some(
    (key) => !(isOwner && (key === 'isAdhoc' || key === 'billable')),
  )
}
...
/**
 * The minutes, spans and start/stop envelope a NEW entry is stored with, from
 * the typed `minutes`, the optional `startAt`/`endAt` pair and the result of
 * {@link normalizeWorkSessions}. `POST /api/time-entries` runs this.
 *
 *   - `sessions` sent: they are the record, so the minutes are their summed
 *     duration and the envelope is theirs (a timer stop, a one-client manual
 *     entry).
 *   - Only `startAt`/`endAt` sent: the typed `minutes` STAND and one session is
 *     made from the pair. A split share relies on this. It carries its source
 *     block's clock in/out so reports can show when the work happened, but it
 *     bills only its own allocation. That is the same shape the server's own
 *     split writes (the block's spans, the share's minutes).
 *   - Neither sent: minutes only, no spans.
 *
 * @param {{minutes: number, startAt?: string, endAt?: string,
 *   sessionsResult: ReturnType<typeof normalizeWorkSessions>}} input
 */
export function resolveCreatedEntryTiming({ minutes, startAt, endAt, sessionsResult }) {
  if (sessionsResult?.sessions && sessionsResult.sessions.length > 0) {
    return {
      minutes: sessionsResult.minutes,
      startAt: sessionsResult.startAt,
      endAt: sessionsResult.endAt,
      sessions: sessionsResult.sessions,
    }
  }
  if (startAt && endAt) {
    return { minutes, startAt, endAt, sessions: [{ startAt, endAt }] }
  }
  return { minutes, startAt, endAt, sessions: [] }
}
