 ...
 */
export function findBlockingWeek(entryWeekStart, priorWeekStarts, submissions) {
  // Only PRIOR weeks that actually have logged time can gate.
  const priorWeeks = [
  ...
  // Oldest → newest: the first prior week that is UN-SUBMITTED (no submission
  // row) or REJECTED (sent back) blocks. A submitted/pending/approved week does
  // NOT block — once submitted it's out of staff's hands, so an awaiting-
  // approval week never locks them out of the timer.
  for (const weekStart of priorWeeks) {
    const status = statusByWeek.get(weekStart)
    if (!status) return { weekStart, reason: 'unsubmitted' }
    if (status === 'rejected') return { weekStart, reason: 'rejected' }
  }
  return null
}
