...
const client = {
  ...
  email: 'ann@acme.com',
  // Longer than the firm's thirty-day window, so this client's own wording is
  // what prints — and 2026-08-01 + 45 days is the fixture's due date.
  paymentTerms: 'Net 45',
...
describe('buildInvoicePdf', () => {
  ...
  it('carries the number, the client, every line label and the total', async () => {
    ...
    expect(text).toContain('Payment terms: Net 45')
  ...
  /*
   * The document tells the client WHEN TO PAY: on receipt, unless their record
   * names a longer window of its own. The invoice's stored due date is the
   * firm's internal past-due line and never reaches the client's page, because a
   * date thirty days out reads as permission to wait (Alex, 2026-09-15).
   */
  it('asks a client with no window of their own to pay on receipt', async () => {
    ...
    expect(text).toContain('Payment terms: Due on receipt')
    expect(text).toContain('On receipt')
    // The firm's internal past-due date is not on the client's document.
    expect(text).not.toContain('September 15, 2026')
  })

  // Shorter than the firm window, so the stored due date was floored to thirty
  // days and the record’s own words would contradict it.
  it('asks a client whose terms are SHORTER than the window to pay on receipt', async () => {
    const buffer = await buildInvoicePdf({
      invoice: invoice(),
