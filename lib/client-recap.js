...
import { billRateAt, costRateFor, latestCostRate, ratePeriodAsOf } from './rate-history.js'
import {
  displayHours,
  laborCost,
  personPeriodCost,
...
export function buildClientRecap(data, opts) {
   ...
   * as a blackout: at the time the firm owner had no rate on file, and on August
   * 2026 production data she had logged time on 31 of the 34 clients with any —
   * so margin, and every plan-vs-actual profit built on it, would have been "—"
   * on 91% of her clients.
   *
   * Nothing here keys off ROLE, and that is deliberate (featreq-6fdd9e98): an
   * owner can now enter her own cost rate on the Team page, and when she has
   * one her hours are priced by the same rule as anyone else's. The rate on
   * file is the whole question; who holds it never was.
   *
  ...
  // period being looked at, through the same `billRateAt` chain the invoice
  // bills by (an unpinned client resolves at the period itself). An estimate
  // is a forecast of work not yet done, so the pin is the defensible price of
  // the plan.
  const estimatePeriod = String(start).slice(0, 7)
  const estimateRatePeriod = ratePeriodAsOf(client, estimatePeriod)
  ...
  const estimateTierRows = byRole.map((row) => {
    ...
    const cost = resolveTierRate(row.tier, latestCostRateOf)
    // Same rule on BOTH sides of the comparison: a tier whose rate resolves to
    // null costs nothing rather than making the estimate unavailable. That is
    // typically the CFO tier, which the owner fills — but only while she has no
    // cost rate on file; once she sets one (featreq-6fdd9e98) the tier prices
    // like any other. `personPeriodCost` returns null for "no
