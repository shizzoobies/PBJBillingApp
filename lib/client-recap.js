...
export function buildClientRecap(data, opts) {
  ...
  const recap = {
    ...
    //
    // Total and Billable are the SUM OF THE byStaff ROWS the card prints, not
    // the rounding of the raw seconds behind them — the two differ, and the one
    // that has to be right is the one she gets by adding the list. `deltaHours`
    // subtracts the two DISPLAYED figures for the same reason: Total, vs-prior
    // and the delta all sit on the same card and have to subtract exactly.
    // adminHours has no row list of its own, so it stays a straight rounding.
    time: {
  ...
  for (const tier of [...ESTIMATE_TIERS, 'Other']) {
    ...
    const actualHours = hours2(tierEntries.reduce((sum, e) => sum + hoursOf(e), 0))
