...
export function buildClientRecap(data, opts) {
  ...
  const billRateOf = (employeeId) => {
    // An unpinned client has no rate period, and `billRateFor(…, null)` answers
    // null for everybody — which would silently drop every versioned rate and
    // fall through to the live mirror. The other two surfaces already read
    // "the pin, else the period being looked at"; this one now does too.
    const versioned = billRateFor(
      billRateVersions,
      employeeId,
      estimateRatePeriod ?? String(start).slice(0, 7),
    )
