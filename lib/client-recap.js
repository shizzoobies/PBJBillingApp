...
export function buildClientRecap(data, opts) {
   ...
   */
  // Still ONE RATE PER PERSON — the newest on file — because `laborCost` hands
  // this only an employee id. Task 11 moves the cost side onto the entry's own
  // date (`costRateFor`), which is the one thing left before a recap's cost is
  // as historically honest as its revenue now is.
  const costRateOf = (employeeId) => latestCostRate(costRateVersions, employeeId)
  /**
