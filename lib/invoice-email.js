 ...
 */
function brandShell({ firmName, contentHtml, footerLines = [] }) {
  // The firm's own contact details under its name. Same fields, same order as
  // the PDF's letterhead (lib/invoice-pdf.js `firmDetailLines`), so the email
  // and the document a client files say the same thing. Blank fields drop out;
  // an unconfigured firm gets the footer it has always had.
  const footerDetail =
    footerLines.length > 0
      ? `<div style="font-size:12px;color:${BRAND.soft};line-height:1.6;padding-top:6px;">${footerLines
          .map((line) => esc(line))
          .join('<br>')}</div>`
      : ''
  return `<!doctype html>
 ...
 * @param {object} [args.firmSettings] the firm's address/phone/email, for the
 *   footer. Omitted -> the footer is the name and the sign-off, as before.
...
export function buildInvoiceEmail({
  ...
  // WHAT THE CLIENT IS TOLD, which is not the invoice's stored due date. That
  // date is the firm's internal past-due line — thirty days from issue — and
  // showing it here would read as permission to wait a month. Unless this
  // client's record names a longer window of its own, they are asked to pay on
  // receipt and no date is printed at all (Alex, 2026-09-15).
  const onReceipt = customerNetDays(client?.paymentTerms) === null
  const due = onReceipt ? '' : longDate(invoice.dueDate)
  ...
  // One plain sentence, in words, saying what is owed and when — above the
  // button, because the panel above states the same two facts as figures in a
  // styled box and a box is the first thing a wary reader distrusts. It says
  // nothing new; it says it the way a person would.
  const dueSentence = onReceipt
    ? `Your total of ${money.format(total)} is due on receipt.`
    : due
      ? `Your total of ${money.format(total)} is due on ${due}.`
      : `Your total for this invoice is ${money.format(total)}.`
  const dueSentenceBlock = `<p style="margin:20px 0 0;font-size:14px;line-height:1.7;color:${BRAND.ink};">${esc(
    dueSentence,
  )}</p>`
    ...
    BRAND.teal
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;border-collapse:collapse;margin-top:20px;">
                <tr><td style="background:${BRAND.panel};border:1px solid ${
                  ...
                  ${
                    onReceipt
                      ? `<div style="font-size:13px;color:${BRAND.ink};padding-top:6px;">Terms: ${esc(
              ...
              ${dueSentenceBlock}${payBlock}${cardBlock}
  ...
  const html = brandShell({ firmName, contentHtml, footerLines: contactLines })
  ...
  const text = [
    ...
    `For ${client?.name ?? ''}${onReceipt ? ' · due on receipt' : due ? ` · due ${due}` : ''}`,
    '',
...
export async function sendInvoicePaymentEmail({
  ...
  const result = await sendEmail({
    ...
    text: email.text,
    attachments,
    // Same sender identity and the same tags as the invoice itself, so all
    // three client emails are one recognizable stream to a spam filter and one
    // traceable thread to the delivery webhook. The log's word for an
    // acknowledgment is 'ack'; Resend's tag spells it out.
    fromName: firmName,
    invoiceId: invoice?.id ?? null,
    kind: kind === 'ack' ? 'payment_ack' : 'receipt',
  })
