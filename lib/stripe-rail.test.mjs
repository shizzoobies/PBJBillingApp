...
/**
 * THE PAYMENT PAGE IS A CLIENT-FACING SURFACE WE DO NOT DRAW.
 *
 * Stripe renders these line items on its own hosted checkout page and repeats
 * them in its own receipt email. For a billing master that makes it a third
 * place a sub company's name could reach KLC — and the one place neither of our
 * own renderers would have caught. Same rule, same negative assertions.
 */
describe('buildCheckoutLineItems — a billing master', () => {
  const master = { id: 'client-master', name: 'KLC Master', isBillingMaster: true }
  const SUB_NAMES = ['Chemtrex', 'XAct', 'Bright Tower', 'Lisa']
  ...
  const namesIn = (items) => JSON.stringify(items)

  it('sends ONE line, named for the month, at the invoice total', () => {
    const items = buildCheckoutLineItems(masterInvoice(), master)
    expect(items).toHaveLength(1)
    expect(items[0].price_data.product_data.name).toBe('Bookkeeping services — August 2026')
    expect(items[0].price_data.unit_amount).toBe(54000)
    // No description at all: there is no per-line detail left to send.
    expect(items[0].price_data.product_data.description).toBeUndefined()
  })

  it('never sends a sub company’s name or coverage window to Stripe', () => {
    const payload = namesIn(buildCheckoutLineItems(masterInvoice(), master))
    for (const name of SUB_NAMES) expect(payload).not.toContain(name)
    expect(payload).not.toContain('Covers')
  })

  it('keeps the card channel clean too, fee line and all', () => {
    const items = buildCardCheckoutLineItems(masterInvoice(), master)
    expect(items).toHaveLength(2)
    expect(items[0].price_data.product_data.name).toBe('Bookkeeping services — August 2026')
    expect(items[1].price_data.product_data.name).toBe(CARD_PROCESSING_FEE_LABEL)
    const payload = namesIn(items)
    for (const name of SUB_NAMES) expect(payload).not.toContain(name)
  })

  // A kept retainer credit is negative, so the existing collapse takes over.
  // That path names only the MASTER and the period, so it is safe by the same
  // standard — asserted rather than assumed.
  it('collapses safely when a retainer credit makes a line negative', () => {
    const items = buildCheckoutLineItems(
      masterInvoice({
        total: 340,
        lineItems: [
    ...
    expect(items).toHaveLength(1)
    expect(items[0].price_data.unit_amount).toBe(34000)
    const payload = namesIn(items)
    for (const name of SUB_NAMES) expect(payload).not.toContain(name)
    expect(payload).toContain('KLC Master')
  })

  it('leaves an ordinary client’s checkout exactly as it was', () => {
    const items = buildCheckoutLineItems(masterInvoice(), client)
    expect(items).toHaveLength(3)
    expect(items[0].price_data.product_data.name).toBe('Billable hours — Lisa (Chemtrex)')
    expect(items[1].price_data.product_data.description).toBe('Covers September 1 – September 30')
  })
})
