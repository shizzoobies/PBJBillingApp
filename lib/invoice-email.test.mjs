...
describe('buildInvoiceEmail', () => {
  ...
  it('states every line and the total', () => {
    ...
    // No date: this client is asked to pay on receipt.
  ...
  it('leads with the amount due and the period in the summary', () => {
    ...
    // This client's record names no window of its own, so the panel states the
    // ASK rather than the firm's internal past-due date.
    expect(html).toContain('Terms: Due on receipt')
  })
...
describe('buildInvoiceEmail — the plain sentence and the firm contact block', () => {
  ...
  it('says what is owed and when, in words, above the pay button', () => {
    ...
    const sentence = 'Your total of $540.00 is due on receipt.'
  ...
  it('drops the date rather than inventing one when the invoice has no due date', () => {
    const { html, text } = buildInvoiceEmail({
      ...
      // Net 45 — a client who WOULD be shown a date, so the missing one is the
      // thing under test rather than the on-receipt wording.
      client: client({ paymentTerms: 'Net 45' }),
    })
...
/**
 * WHAT THE CLIENT IS ASKED FOR (Alex, 2026-09-15).
 *
 * The invoice's stored due date is the firm's own past-due line - thirty days
 * from issue, the day after which it gets chased. The client is asked to pay on
 * receipt and is never shown that date, because a date reads as permission to
 * wait. The exception is a client whose record names a LONGER window of its
 * own: that window was promised to them, so their invoice carries its date.
 */
describe('buildInvoiceEmail - the payment ask', () => {
  it('asks a client with no terms of their own to pay on receipt, with no date', () => {
    const { html, text } = buildInvoiceEmail({ invoice: invoice(), client: client() })

    expect(html).toContain('Terms: Due on receipt')
    expect(html).toContain('Your total of $540.00 is due on receipt.')
    expect(text).toContain('Your total of $540.00 is due on receipt.')
    expect(text).toContain('due on receipt')
    // The firm-internal date must not reach the inbox anywhere.
    expect(html).not.toContain('September 30, 2026')
    expect(text).not.toContain('September 30, 2026')
    expect(html).not.toContain('Due date')
  })

  it('asks a "Due on receipt" record for payment on receipt', () => {
    const { html } = buildInvoiceEmail({
      invoice: invoice(),
      client: client({ paymentTerms: 'Due on receipt' }),
    })
    expect(html).toContain('Terms: Due on receipt')
    expect(html).not.toContain('September 30, 2026')
  })

  // Shorter than the firm window, so it buys the client nothing: the stored due
  // date was floored to thirty days, and printing "Net 15" beside that date
  // would contradict it. They are asked to pay on receipt like everybody else.
  it('asks a client whose terms are SHORTER than the window to pay on receipt', () => {
    const { html } = buildInvoiceEmail({
      invoice: invoice(),
      client: client({ paymentTerms: 'Net 15' }),
    })
    expect(html).toContain('Terms: Due on receipt')
    expect(html).not.toContain('September 30, 2026')
  })

  it('gives a client with a LONGER window their own date, in both renderings', () => {
    const { html, text } = buildInvoiceEmail({
      invoice: invoice(),
      client: client({ paymentTerms: 'Net 45' }),
    })
    expect(html).toContain('Due date: September 30, 2026')
    expect(html).toContain('Your total of $540.00 is due on September 30, 2026.')
    expect(text).toContain('due September 30, 2026')
    expect(html).not.toContain('due on receipt')
  })
})

// Net 45 on the record: the one client in this file whose invoice carries a
// due DATE rather than an ask to pay on receipt.
const masterClient = (over = {}) =>
  client({ id: 'client-master', name: 'KLC Master', isBillingMaster: true, paymentTerms: 'Net 45', ...over })
...
/**
 * THE DURABLE PAY LINK in the email.
 *
 * `buildInvoiceEmail` takes `payUrl` as an opaque string and always has, so
 * nothing in it had to change — which is exactly why this is worth pinning. The
 * URL is no longer Stripe's; it is `app.pbjsa.com/pay/<token>`, and the token is
 * base64url, so it can carry `-` and `_`. A renderer that escaped, trimmed,
 * lower-cased or line-wrapped any of that would produce a link that looks right
 * in the email and 404s when the client clicks it.
 *
 * The HTML href and the plain-text line must ALSO be identical: some clients
 * read one and some read the other, and two different URLs for one invoice is
 * two different answers to "is this real?".
 */
describe('the durable pay link survives both renderers verbatim', () => {
  // A real 43-character base64url token, with both of the characters that make
  // base64url different from base64 in it.
  const DURABLE_URL = 'https://app.pbjsa.com/pay/Qk3z-vN9_pY2sLd4TgH1xWmC8rEbA6uJfKq0nZi5-Tw'

  it('appears unchanged in the HTML href', () => {
    const { html } = buildInvoiceEmail({
      invoice: invoice(),
      client: client(),
      payUrl: DURABLE_URL,
    })
    expect(html).toContain(`href="${DURABLE_URL}"`)
  })

  it('appears unchanged in the plain-text line', () => {
    const { text } = buildInvoiceEmail({
      invoice: invoice(),
      client: client(),
      payUrl: DURABLE_URL,
    })
    expect(text).toContain(`Pay by bank transfer: ${DURABLE_URL}`)
  })

  it('is the same URL in both parts', () => {
    const { html, text } = buildInvoiceEmail({
      invoice: invoice(),
      client: client(),
      payUrl: DURABLE_URL,
    })
    const inHtml = [...html.matchAll(/href="(https:\/\/app\.pbjsa\.com\/pay\/[^"]+)"/g)].map(
      (match) => match[1],
    )
    const inText = [...text.matchAll(/(https:\/\/app\.pbjsa\.com\/pay\/\S+)/g)].map(
      (match) => match[1],
    )
    expect(inHtml.length).toBeGreaterThan(0)
    expect(inText.length).toBeGreaterThan(0)
    expect(new Set([...inHtml, ...inText])).toEqual(new Set([DURABLE_URL]))
  })

  // Unchanged from before: no link, no button. A re-send of a paid or
  // still-clearing invoice must not invite a second payment.
  it('still shows no button at all when there is no link', () => {
    const { html, text } = buildInvoiceEmail({ invoice: invoice(), client: client() })
    expect(html).not.toContain('/pay/')
    expect(text).not.toContain('Pay by bank transfer')
  })
})
