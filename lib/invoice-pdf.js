...
function drawInvoice(doc, { invoice, client, firmSettings }) {
  ...
  // "Due" is what the CLIENT is asked for, not the invoice's stored due date:
  // that date is the firm's own past-due line and printing it would read as
  // thirty days' grace. A client whose record names a longer window of its own
  // still gets their date, because that window is the one they were promised.
  const dueField =
    customerNetDays(client?.paymentTerms) === null ? 'On receipt' : longDate(invoice?.dueDate)
  ...
  /* ---- terms, note, footer -------------------------------------------- */
  // What the client is asked for — "Due on receipt" for everyone whose record
  // names no longer window of its own, which is most of them. It agrees with
  // the "Due" field above by construction: both branch on `customerNetDays`.
  const terms = paymentTermsLabel(client?.paymentTerms)
  if (terms) {
