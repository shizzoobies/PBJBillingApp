...
import { paymentTermsLabel } from './invoice-draft.js'
...
function drawInvoice(doc, { invoice, client, firmSettings }) {
  ...
  /* ---- terms, note, footer -------------------------------------------- */
  // The terms the invoice was actually DATED by, not the stored wording. The
  // four KLC-group records say "Due on receipt" and her markup reads "Due on
  // Demand" — printing either of those beside a due date thirty days out would
  // contradict the document's own date. A client whose stored terms are longer
  // than the firm's window still prints their own words (`paymentTermsLabel`).
