...
describe('a null rate is NO rate, not $0', () => {
  // The columns are NOT NULL and the endpoints never write one, but
  // `Number(null)` is 0 and 0 passes `Number.isFinite` - so without an explicit
  // type check a bad row would silently bill or cost an hour at $0.00 instead
  // of falling through to the next step of the chain.
  const nullRow = [{ userId: 'emp-lisa', effectivePeriod: '2026-06', rate: null }]
  const nullCost = [{ userId: 'emp-lisa', effectiveDate: '2026-06-01', rate: null }]

  it('billRateFor answers null, not 0', () => {
    expect(billRateFor(nullRow, 'emp-lisa', '2026-06')).toBeNull()
  })

  it('costRateFor answers null, not 0', () => {
    expect(costRateFor(nullCost, 'emp-lisa', '2026-06-15')).toBeNull()
  })

  it('latestBillRate answers null, not 0', () => {
    expect(latestBillRate(nullRow, 'emp-lisa')).toBeNull()
  })

  it('billRateAt’s first-rate step answers null, not 0', () => {
    expect(billRateAt(nullRow, { id: 'emp-lisa', billRate: 40 }, '2026-01', '2026-09')).toBeNull()
  })

  it('a numeric string rate is still not a number', () => {
    expect(billRateFor([{ ...nullRow[0], rate: '40' }], 'emp-lisa', '2026-06')).toBeNull()
  })
})
